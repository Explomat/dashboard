...
/******/ 	__webpack_require__.p = "/x5__adaptation-dashboard/client/";
/******/
...
var Element = function Element(key, type, props) {
	return {
		key: key,
		type: type,
		props: props || {}
...
Element.createElement = function (type, config, children) {
	var propName = void 0;
	var props = {};
	var key = null;
	...
	if (config) {
		...
		for (propName in config) {
			if (hasOwnProperty.call(config, propName) && !EXCLUDED_PROPS.hasOwnProperty(propName)) {
...
Element.createFactory = function (type) {
	var factory = Element.createElement.bind(null, type);
	factory.type = type;
	return factory;
};
...
var elements = tags.reduce(function (f, s) {
	f[s] = _Element2.default.createFactory(s);
	return f;
}, {});

exports.default = _extends({}, elements);
...
var _eventList = __webpack_require__(140);
...
function update(id) {
	var obj = components[id];
	var parentDomNode = obj.parent && obj.parent.domNode ? obj.parent.domNode : rootDomNode;
	_renderTree(obj, parentDomNode);
}

function render(_ref, container) {
	var type = _ref.type,
	    props = _ref.props,
	    parent = _ref.parent,
	    id = _ref.id;
	...
	_renderTree({ type: type, props: props, parent: parent, id: id }, container);
}
...
function _getComponentForNode(id) {
	var node = nodes[id];
	if (node && typeof node.type === 'string' && node.parent) {
...
function _clearNode(id) {
	...
	if (node && node.domNode) {
		if (node.domNode.hasAttributes()) {
			var attrs = node.domNode.attributes;
		...
		if (node.props) {
			for (var p in node.props) {
				if (node.props.hasOwnProperty(p) && _eventList2.default.hasOwnProperty(p)) {
					var ev = p.substr(2).toLowerCase();
...
function _renderTree(_ref2, parentDomNode) {
	var key = _ref2.key,
	    type = _ref2.type,
	    props = _ref2.props,
	    parent = _ref2.parent,
	...
	if (parent && typeof parent.type === 'function') {
		...
		var _id = id !== undefined ? id.toString() : '0';

		var _id$split = _id.split('$'),
		    _id$split2 = _slicedToArray(_id$split, 1),
		    oid = _id$split2[0];

		if (~oid.indexOf('.')) {
			var lastIndex = oid.lastIndexOf('.');
			oid = oid.substr(lastIndex + 1);
		}
		//oid[(oid.length - 1)];
		var okey = key !== undefined && key !== null ? '$' + key : '';

		var parentId = parent ? parent.id.toString() : '0';

		var _parentId$split = parentId.split('$'),
		    _parentId$split2 = _slicedToArray(_parentId$split, 1),
		    pid = _parentId$split2[0];
	...
	if (typeof type === 'function') {
		var isSameElement = components.hasOwnProperty(newId) && components[newId].type === type;
		var Component = null;
		...
		if (el) {
			var child = _renderTree(_extends({}, el, {
				parent: obj,
				id: '0'
			}), parentDomNode);
		...
		if (props) {
			...
			if (props.children !== undefined) {
				if (Array.isArray(props.children)) {
					...
					for (i = 0, len = props.children.length; i < len; i++) {
						...
						if (ch) {
							childs.push(_renderTree(_extends({}, ch, {
								parent: obj,
								id: i
							}), obj.domNode));
						}
					...
					for (j = obj.domNode.childNodes.length - 1; j >= 0; j--) {
						var childDomNode = obj.domNode.childNodes[j];
						var isEqual = false;
						for (k = childs.length - 1; k >= 0; k--) {
							var equalNode = childs[k].domNode || childs[k].instance.domNode;
						...
						if (!isEqual) {
							if (key === undefined || key === null) {
								var dataId = childDomNode.getAttribute('data-id');
								_removeNode(dataId);
							}
					...
					_renderTree(_extends({}, props.children, {
						parent: obj,
						id: '0'
					}), obj.domNode);
					...
					var text = props.children.toString();
					if (type === _tags2.default.textarea.type) {
			...
			if (props.ref) {
				var curComponent = _getComponentForNode(newId);
				if (curComponent) {
...
var _Task = __webpack_require__(154);
...
exports.selectTaskStatus = selectTaskStatus;
...
function getState() {
	(0, _dispatcher.dispatch)({ type: _constants2.default.DASHBOARD_GET_STATE });

	var path = _config.url.createPath({
		server_name: 'adaptation',
		action_name: 'Adaptations'
	});

	(0, _ajax.get)(path).then(function (resp) {
		return JSON.parse(resp);
	}).then(function (data) {
		if (data.error) {
			(0, _dispatcher.dispatch)({
				type: _constants2.default.DASHBOARD_GET_STATE_FAILURE,
				error: data.error
			});
		} else {
			(0, _dispatcher.dispatch)({
				type: _constants2.default.DASHBOARD_GET_STATE_SUCCESS,
				state: data
			});
		}
	}).catch(function (e) {
...
function selectBlock(blockId) {
	(0, _dispatcher.dispatch)({
		type: _constants2.default.DASHBOARD_SELECT_BLOCK,
		blockId: blockId
	});
}

function selectTaskStatus(taskStatus) {
	(0, _dispatcher.dispatch)({
		type: _constants2.default.DASHBOARD_SELECT_TASK_STATUS,
...
exports.dom = exports.url = undefined;

var _servers2 = __webpack_require__(141);

var routerId = '6424386388863752846';
var customBaseUrl = '/custom_web_template.html';

(0, _servers2.addServer)({ id: '6465561911442173706', name: 'adaptation' }).addActions(['Adaptations']);

var url = {
	getServerId: function getServerId(serverName, actionName) {
		var _servers = (0, _servers2.getAll)().filter(function (s) {
			var actions = s.getActions().filter(function (action) {
...
var _createRemoteActions = __webpack_require__(138);

var _createRemoteActions2 = _interopRequireDefault(_createRemoteActions);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var remoteConstants = (0, _createRemoteActions2.default)(['DASHBOARD_GET_STATE']);
...
exports.default = Object.assign(remoteConstants, constants);
...
exports.default = createRemoteActions;
...
function createRemoteActions(actions) {
	...
	actions.forEach(function (action) {
		var reducedAction = createAction(action);
...
(0, _render.render)((0, _dashboard2.default)(), document.getElementById('app'));
...
function Server(srv) {
	...
	var id = srv.id || iter++;
	var name = srv.name || 'name_' + srv.id;
	var actions = [];
...
var _eventEmmiter = __webpack_require__(144);
...
function addEventListener(cb) {
	(0, _eventEmmiter.subscribe)('update', cb);
}

(0, _dispatcher.register)(function dashboardStore(action) {
	switch (action.type) {
		case _constants2.default.DASHBOARD_GET_STATE:
			{
				state = _extends({}, state, {
					isFetching: true
				});
				(0, _eventEmmiter.emit)('update', state);
				break;
			}
		case _constants2.default.DASHBOARD_GET_STATE_SUCCESS:
			{
				var countTasksByStatus = {};
				var tasks = [];
				var taskStatuses = {};
				action.state.blocks.reduce(function (f, s) {
					var t = f.sort(function (a, b) {
				...
				var activeTaskStatus = Object.keys(taskStatuses)[0];
				var filteredTasks = tasks.filter(function (t) {
					return t.status === activeTaskStatus;
				});
				state = _extends({}, state, action.state, {
					tasks: tasks,
		...
		case _constants2.default.DASHBOARD_GET_STATE_FAILURE:
			{
				state = _extends({}, state, {
					error: action.error,
					isFetching: false
		...
		case _constants2.default.DASHBOARD_SELECT_BLOCK:
			{
				state = _extends({}, state, {
					activeBlock: state.blocks.filter(function (b) {
						return b.id.toString() === action.blockId.toString();
					})[0]
				});
				(0, _eventEmmiter.emit)('update', state);
				break;
			}
		case _constants2.default.DASHBOARD_SELECT_TASK_STATUS:
			{
				state = _extends({}, state, {
					activeTaskStatus: action.taskStatus,
					filteredTasks: state.tasks.filter(function (t) {
						return t.status === action.taskStatus;
					})
				});
				(0, _eventEmmiter.emit)('update', state);
				break;
...
var BlockHeader = function (_Component) {
	...
	_createClass(BlockHeader, [{
		...
		value: function render() {
			...
			var classes = 'block-header ' + (isActive ? 'block-header--active' : '');
			return _tags2.default.span({
				class: classes,
				style: 'width: ' + width + '%;',
...
var BlockHeaderElement = _Element2.default.createFactory(BlockHeader);
exports.default = BlockHeaderElement;
...
var _BlockHeader = __webpack_require__(145);

var _BlockHeader2 = _interopRequireDefault(_BlockHeader);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = _BlockHeader2.default;
...
var Block = exports.Block = function (_Component) {
	...
	_createClass(Block, [{
		...
		value: function render() {
			...
			return _tags2.default.div({
				class: 'block'
			}, [_tags2.default.div({
				class: 'block__tasks'
			}, tasks.map(function (t) {
				return (0, _task2.default)(t);
			}))]);
		}
	}]);
...
var BlockElement = _Element2.default.createFactory(Block);
...
var _block = __webpack_require__(148);
...
var _blockHeader = __webpack_require__(146);

var _blockHeader2 = _interopRequireDefault(_blockHeader);

var _taskHeader = __webpack_require__(153);
...
var _dashboardStore = __webpack_require__(142);
...
function updateState(st) {
	this.setState(_extends({}, this.state, st));
}
...
var Dashboard = exports.Dashboard = function (_Component) {
	...
	function Dashboard(props) {
		...
		_this.handleSelectBlock = _this.handleSelectBlock.bind(_this);
		_this.handleSelectTaskStatus = _this.handleSelectTaskStatus.bind(_this);
		return _this;
	}
	...
	_createClass(Dashboard, [{
		...
		value: function componentWillMount() {
			(0, _dashboardStore.addEventListener)(updateState.bind(this));
			(0, _actions.getState)();
		}
	}, {
		key: 'handleSelectBlock',
		value: function handleSelectBlock(blockId) {
			(0, _actions.selectBlock)(blockId);
		}
	}, {
		key: 'handleSelectTaskStatus',
		value: function handleSelectTaskStatus(taskStatus) {
			(0, _actions.selectTaskStatus)(taskStatus);
		}
	}, {
		key: 'render',
		value: function render() {
   	...
   	tags.div({
   		class: 'dashboard__footer-taks'
			...
			var _state = this.state,
			    ...
			    statusTitle = _state.statusTitle,
			    planReadinessDate = _state.planReadinessDate,
			    finishDate = _state.finishDate,
			    blocks = _state.blocks,
			    activeBlock = _state.activeBlock,
			    filteredTasks = _state.filteredTasks,
			    countTasksByStatus = _state.countTasksByStatus,
			    taskStatuses = _state.taskStatuses,
			    activeTaskStatus = _state.activeTaskStatus,
			...
			var wb = Math.round(100 / blocks.length);
			var statusClasses = 'dashboard-container__status-value dashboard-container__status-value--' + status;
			return _tags2.default.div({
				...
				return (0, _blockHeader2.default)({
					id: b.id,
					name: b.name,
					width: wb,
					onClick: _this2.handleSelectBlock,
					isActive: (activeBlock || {}).id === b.id
				});
			}))), _tags2.default.div({
				class: 'dashboard__body'
			}, activeBlock && (0, _block2.default)(activeBlock)), _tags2.default.div({
				class: 'dashboard__footer'
			}, [_tags2.default.div({
				class: 'dashboard__footer-task-statuses'
			}, Object.keys(taskStatuses).map(function (cs) {
				return (0, _taskHeader2.default)({
					id: cs,
					name: taskStatuses[cs],
					count: countTasksByStatus[cs],
					isActive: activeTaskStatus === cs,
					onClick: _this2.handleSelectTaskStatus
				});
			})), _tags2.default.div({
				class: 'dashboard__footer-taks'
...
var DashboardElement = _Element2.default.createFactory(Dashboard);
exports.default = DashboardElement;
	...
	_createClass(Error, [{
		...
		value: function render() {
			var _props = this.props,
			    ...
			    rest = _objectWithoutProperties(_props, ['text', 'className']);

			return _tags2.default.div(_extends({
...
var ErrorElement = _Element2.default.createFactory(Error);
...
var TaskHeader = function (_Component) {
	_inherits(TaskHeader, _Component);
	...
	_createClass(TaskHeader, [{
		...
		value: function render() {
			...
			var classes = 'task-header ' + (isActive ? 'task-header--active' : '');
			return _tags2.default.span({
				class: classes,
				onClick: this.handleClick
			}, [_tags2.default.span({
				class: 'task-header__name'
			}, name), _tags2.default.span({
...
var TaskHeaderElement = _Element2.default.createFactory(TaskHeader);
exports.default = TaskHeaderElement;
...
var _TaskHeader = __webpack_require__(152);

var _TaskHeader2 = _interopRequireDefault(_TaskHeader);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = _TaskHeader2.default;
	...
	_createClass(Task, [{
		...
		value: function render() {
			var _props = this.props,
			    name = _props.name,
			    href = _props.href,
			    startUsageDate = _props.startUsageDate,
			    lastUsageDate = _props.lastUsageDate,
			    status = _props.status,
			    statusTitle = _props.statusTitle;

			var statusClasses = 'task__status-title task__status-title--' + status + ' ';
			statusClasses += '' + (startUsageDate || lastUsageDate ? '' : 'task__status-title--without-date');
			return _tags2.default.div({
				class: 'task'
			}, [href ? _tags2.default.a({
				class: 'task__name',
				target: '__blank',
				href: href.replace(/&amp;/g, '&')
			}, name) : _tags2.default.span({
				class: 'task__name'
			}, name), _tags2.default.div({
				class: 'task__body'
			}, [startUsageDate && _tags2.default.span({
				class: 'task__start-usage-date'
			}, new Date(startUsageDate).toLocaleString('ru', {
				year: 'numeric',
				month: 'numeric',
				day: 'numeric'
			})), lastUsageDate && _tags2.default.span(null, ' - '), lastUsageDate && _tags2.default.span({
...
var TaskElement = _Element2.default.createFactory(Task);
...
/***/ })
/******/ ]);
//# sourceMappingURL=2f181185485e763611da.bundle.js.map